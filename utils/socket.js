...
var webSocket = {
  ...
  connectSocket: function(options) {
    ...
    socketMsgQueue = []
    wx.connectSocket({
      url: app.globalData.WSSAddress + app.globalData.userId+'_'+options.roomNum,
    ...
    self.sendSocketMessage({
      ...
      fail: function(res) {
        ...
        if (heartBeatFailCount > 2) {
          // 重连
          let _roomNum = wx.getStorageSync('roomNum')
          self.connectSocket({roomNum: _roomNum});
        }
        if (heart) {
          heartBeatTimeOut = setTimeout(() => {
            self.heartBeat();
...
wx.onSocketClose(function(res) {
  ...
  if (!socketClose) {
    ...
    connectSocketTimeOut = setTimeout(() => {
      let _roomNum = wx.getStorageSync('roomNum')
      webSocket.connectSocket({roomNum: _roomNum});
    }, 3000);
  }
})
